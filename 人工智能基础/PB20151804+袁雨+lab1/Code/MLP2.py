...
# 建立一个二层感知机网络
class MLP2(torch.nn.Module):  # 继承 torch 的 Module
    def __init__(self):
        super(MLP2, self).__init__()  #
        # 初始化三层神经网络 两个全连接的隐藏层，一个输出层
        hidden1 = 2048  # 第二层节点数
        self.fc1 = torch.nn.Linear(in_num, hidden1)  # 第一个隐含层
        self.fc2 = torch.nn.Linear(hidden1, out_num)  # 输出层
        # 使用dropout防止过拟合
...
def main():
    ...
    print('Accuracy of the network  on the valid_data: %.6f %%' % test(valid_loader))
