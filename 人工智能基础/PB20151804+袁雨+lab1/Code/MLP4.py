...
#             self.root = r'D:\PytorchProject\lab1\Lab1_train.csv'
#         data = pd.read_csv(self.root)   # 从CSV文件中加载原始数据集
#         label = data.iloc[:, -1]  # 装载标签
#         self.label = label[0:1000]
#         data = np.array(data.iloc[:, 1:-1])    # 装载数据
...
# 建立一个四层感知机网络
class MLP4(torch.nn.Module):  # 继承 torch 的 Module
    def __init__(self):
        ...
        hidden2 = 1536  # 第三层节点数
        hidden3 = 1024
        self.fc1 = torch.nn.Linear(in_num, hidden1)  # 第一个隐含层
        self.fc2 = torch.nn.Linear(hidden1, hidden2)  # 第二个隐含层
        self.fc3 = torch.nn.Linear(hidden2, hidden3)  # 第三个隐含层
        self.fc4 = torch.nn.Linear(hidden3, out_num)  # 输出层
        # 使用dropout防止过拟合
...
def main():
    ...
    for i, (data, label) in enumerate(valid_loader):
        ...
        valid_pre.extend(np.argmax(pre, axis=1))  # 求每一行的最大值索引
        valid_label.extend(label.cpu())
    print('Accuracy of the network  on the test_data: %.6f %%' % test(test_loader))
    print("F1 Score:{:.6f}".format(metrics.f1_score(valid_label, valid_pre)))
    print("测试集：")
    test_pre = []
    test_label = []
    test_prob = []
