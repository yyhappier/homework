...
//	srand((unsigned)time(NULL));
	complex<double> f=complex<double>(0.7*sin(2*PI*2*t)+sin(2*PI*5*t)+0.3*rand()/(RAND_MAX+1.0),0);
	return f;
}
...
//		cout<<length[i]<<endl;
	...
	for(i=0;i<n;i++)
	{
        if(i<n*0.125||i>=n*0.875)
                g_s[i]=g[i];
        else 
			g_s[i]=complex<double >(0.0 , 0.0);
	}
    complex<double>* g_s_inverse;
    g_s_inverse=new complex<double>[n];
    g_s_inverse=IFFT(g_s,n);
    cout<<"smooth"<<endl;
    for(int i=0;i<n;i++) 
		cout<<g_s_inverse[i].real()<<endl;
	return 0;
}